...
t_eval_result mx_eval(t_exp exp, t_global_environment *gv,
    ...
    if (exp == NULL || exp[0] == '\0' || exp[0] == '\n')
        return NULL;
    exp_type = mx_get_expressiontype_by_id(mx_get_binary_opid(exp));
    if (exp_type == variable_assignment)  // name=Value
        result = mx_eval_assignment(exp, gv);
    else if (exp_type == pipeline)        // simple_command | simple_command
        result = mx_eval_seq_pipeline(exp, gv, NULL);
    else if (exp_type == sublist)         // pipeline || pipeline && pipeline
        result = mx_eval_logic(exp, gv);
    else if (exp_type == list)            // sublist ; sublist ; sublist ;
        result = mx_eval_sequence(exp, gv);
    else if (exp_type == simple_command)  //simple_command:
...
//
//// eval_assignment
//t_exp _assignment_variable(t_exp exp) {
//    char *car = NULL;
//    car = mx_left_exp(exp, "=");
...
//}
//
//t_exp _assignment_value(t_exp exp) {
...
//}
//
//t_exp _words_after_assignment(t_exp exp) {
//    char *cdr = mx_right_exp(exp, "=");
...
//            command = _command(exp);
//            operands = _operands(exp);
